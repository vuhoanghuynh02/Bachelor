...
-- RÀNG BUỘC KQUA
ALTER TABLE KETQUATHI
ADD CONSTRAINT CHK_KQUA
CHECK ((KQUA = N'Đạt' AND DIEM BETWEEN  5 AND 10) OR (KQUA = N'Không đạt' AND DIEM < 5));

-- I. 6. Học viên thi một môn tối đa 3 lần.-- BẢNG KETQUATHI-- RÀNG BUỘC LANTHIALTER TABLE KETQUATHIADD CONSTRAINT TOIDA3LANTHICHECK (LANTHI <= 3);-- I. 7. Học kỳ chỉ có giá trị từ 1 đến 3.
-- BẢNG GIANGDAY
-- RÀNG BUỘC HOCKY
ALTER TABLE GIANGDAY
ADD CONSTRAINT HOCKYTUCHI1DEN3
...
-- TRIGGER LOP
-- ỔN
-- KHÔNG CẦN TRIGGER CHO DELETE BỞI VÌ KHÔNG CÓ YÊU CẦU MỖI LỚP PHẢI CÓ MỘT LỚP TRƯỞNG
CREATE TRIGGER TRGLOPPHAITHUOCLOP ON LOP
FOR INSERT, UPDATE
AS
BEGIN
	IF (NOT EXISTS (
...
-- TRIGGER KHOA
-- ỔN
-- DELETE KHÔNG VI PHẠM RÀNG BUỘC
CREATE TRIGGER TRIGGER_TRUONGKHOA ON KHOA
AFTER INSERT, UPDATE AS
BEGIN
	DECLARE @TRGKHOA CHAR(4), @HOCVI  VARCHAR(10);
...
INSERT INTO KHOA (MAKHOA, TRGKHOA)
VALUES ('HAHA', 'GV05');
...
-- DỄ
ALTER TABLE MONHOC
ADD CONSTRAINT TCLTTHCHECHLECHKHONGQUA3
CHECK (ABS(TCLT - TCTH) <= 3);

-- I. 15. Học viên chỉ được thi một môn học nào đó khi lớp của học viên đã học xong môn học này.
-- BẢNG GIANGDAY GD, KETQUATHI KQT, HOCVIEN HV
-- RÀNG BUỘC GD.DENNGAY, KQT.NGTHI => TRIGGER CẢ 2 BẢNG
-- KHÓ
-- UPDATE, DELETE GD CÓ ẢNH HƯỞNG ĐẾN RÀNG BUỘC
-- INSERT, UPDATE KQT CÓ ẢNH HƯỞNG ĐẾN RÀNG BUỘC
CREATE TRIGGER TRIGGER_GD_DENNGAY ON GIANGDAY
FOR UPDATE, DELETE AS
BEGIN
	DECLARE @NEWDENNGAY SMALLDATETIME;	
	SELECT @NEWDENNGAY = DENNGAY
	FROM INSERTED;
	IF EXISTS (
		SELECT *
		FROM DELETED D 
			JOIN HOCVIEN HV ON HV.MALOP = D.MALOP
			JOIN KETQUATHI KQT ON KQT.MAHV = HV.MAHV
		WHERE NOT (@NEWDENNGAY >= KQT.NGTHI) AND  D.DENNGAY >= KQT.NGTHI AND D.MAMH = KQT.MAMH
		)
	BEGIN
...
CREATE TRIGGER TRIGGER_KQT_NGTHI ON KETQUATHI
...
BEGIN
	IF EXISTS (
		...
		WHERE GD.DENNGAY >= I.NGTHI AND GD.MAMH = I.MAMH
		)
	BEGIN
		PRINT N'Học viên chỉ được thi một môn học nào đó khi lớp của học viên đã học xong môn học này.'
		ROLLBACK TRANSACTION
...
-- TRIGGER GIANGDAY
-- ỔN, VỪA SỨC
-- INSERT CÓ ẢNH HƯỞNG ĐẾN RÀNG BUỘC
CREATE TRIGGER TRIGGER_TOIDA3MON ON GIANGDAY
AFTER INSERT AS
BEGIN
	IF EXISTS (
		SELECT *
...
-- TRIGGER LOP, HOCVIEN
-- TRÊN TRUNG BÌNH, DÀI
-- INSERT, UPDATE, DELETE HOCVIEN CÓ ẢNH HƯỞNG ĐẾN RÀNG BUỘC
-- INSERT, UPDATE LOP CÓ ẢNH HƯỞNG ĐẾN RÀNG BUỘC
-- NẾU RÀNG BUỘC HOCVIEN SẼ KHÔNG THỂ INSERT ĐƯỢC, DO INSERT SẼ LÀM TĂNG SISO LÊN 1
-- GIẢI PHÁP: CẬP NHẬT SISO KHI INSERT, UPDATE, DELETE HOCVIEN

-- I. 18. Trong quan hệ DIEUKIEN giá trị của thuộc tính MAMH và MAMH_TRUOC trong cùng
-- một bộ không được giống nhau (“A”,”A”) và cũng không tồn tại hai bộ (“A”,”B”) và (“B”,”A”).
-- BẢNG DIEUKIEN
-- TRIGGER
-- ỔN
-- INSERT, UPDATE CÓ ẢNH HƯỞNG ĐẾN RÀNG BUỘC
CREATE TRIGGER TRIGGER_DIEUKIEN ON DIEUKIEN
...
BEGIN
	...
	IF EXISTS (
		SELECT *
		FROM DIEUKIEN
		WHERE (MAMH = @MAMH AND MAMH_TRUOC = @MAMH) 
			OR (MAMH = @MAMH_TRUOC AND MAMH_TRUOC = @MAMH_TRUOC) 
			OR (MAMH = @MAMH_TRUOC AND MAMH_TRUOC = @MAMH)
		)
	BEGIN
		ROLLBACK TRANSACTION
	END
...
-- DỄ
-- INSERT, UPDATE CÓ ẢNH HƯỞNG ĐẾN RÀNG BUỘC
CREATE TRIGGER TRIGGER_MUCLUONG ON GIAOVIEN
FOR INSERT, UPDATE AS
BEGIN
	IF EXISTS (
		SELECT *
		FROM GIAOVIEN GV, INSERTED I
		WHERE GV.HOCVI = I.HOCVI AND GV.HOCHAM = I.HOCHAM AND GV.HESO = I.HESO AND GV.MUCLUONG <> I.MUCLUONG
		)
	BEGIN
		ROLLBACK TRANSACTION
	END
...
-- TRIGGER
-- DỄ
-- INSERT, UPDATE, DELETE CÓ ẢNH HƯỞNG ĐẾN RÀNG BUỘC
-- ĐỀ KHÔNG YÊU CẦU CÙNG MÔN => TẤT CẢ CÁC MÔN ĐỀU PHẢI DƯỚI 5
CREATE TRIGGER TRIGGER_IU_THILAI ON KETQUATHI
AFTER INSERT, UPDATE AS
BEGIN
	IF EXISTS (
		SELECT *
		FROM KETQUATHI KQT JOIN INSERTED I ON I.MAHV = KQT.MAHV
		WHERE I.DIEM >= 5 AND I.LANTHI < KQT.LANTHI
		)
	BEGIN
		ROLLBACK TRANSACTION
	END
...
CREATE TRIGGER TRIGGER_D_THILAI ON KETQUATHI
AFTER DELETE AS
BEGIN
	IF EXISTS (
		SELECT *
		FROM KETQUATHI KQT JOIN INSERTED I ON I.MAHV = KQT.MAHV
		WHERE KQT.LANTHI > I.LANTHI
		)
	BEGIN
		ROLLBACK TRANSACTION
	END
...
-- DỄ
-- INSERT, UPDATE CÓ ẢNH HƯỞNG ĐẾN RÀNG BUỘC
CREATE TRIGGER TRIGGER_NGTHISAULONHONTRC ON KETQUATHI
AFTER INSERT, UPDATE AS
BEGIN
	IF EXISTS (
		SELECT *
		FROM KETQUATHI KQT JOIN INSERTED I ON I.MAHV = KQT.MAHV AND I.MAMH = KQT.MAMH
		WHERE I.LANTHI > KQT.LANTHI AND I.NGTHI <= KQT.NGTHI
		)
	BEGIN
		ROLLBACK TRANSACTION
	END
...
-- I. 23. Khi phân công giảng dạy một môn học, phải xét đến thứ tự trước sau giữa các môn học (sau
-- khi học xong những môn học phải học trước mới được học những môn liền sau).
-- BẢNG DIEUKIEN, GIANGDAY
-- TRIGGER DIEUKIEN, GIANGDAY
-- TRÊN TRUNG BÌNH, DÀI
-- INSERT DIEUKIEN CÓ ẢNH HƯỞNG ĐẾN RÀNG BUỘC
-- INSERT, UPDATE, DELETE GIANGDAY CÓ ẢNH HƯỞNG ĐẾN RÀNG BUỘC

-- I. 24. Giáo viên chỉ được phân công dạy những môn thuộc khoa giáo viên đó phụ trách.
-- BẢNG GIANGDAY, GIAOVIEN, MONHOC
-- TRIGGER GIANGDAY, GIAOVIEN, MONHOC
-- TRÊN TRUNG BÌNH, DÀI
-- INSERT, UPDATE GIANGDAY (VD SỬA MAGV) CÓ ẢNH HƯỞNG ĐẾN RÀNG BUỘC
-- UPDATE GIAOVIEN (VD SỬA MAKHOA) CÓ ẢNH HƯỞNG ĐẾN RÀNG BUỘC
-- UPDATE MONHOC (VD SỬA MAKHOA) CÓ ẢNH HƯỞNG ĐẾN RÀNG BUỘC
